...
import { 
  ...
  ShoppingCart, MessageSquare, Bell, 
...
const navItems = [
  ...
  { name: 'Add Medicine', path: '/seller/add', icon: PlusCircle },
          ...
          <AnimatePresence mode="popLayout">
            {!isCollapsed && (
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                className="font-bold text-xl bg-clip-text text-transparent bg-gradient-to-r from-success to-primary whitespace-nowrap"
              >
                MediCycle Seller
              </motion.div>
            )}
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={cn('w-5 h-5 shrink-0', isActive ? 'text-success' : 'text-white/50 group-hover:text-white/80')} />
                {!isCollapsed && (
        ...
        <div className="p-4 border-t border-white/5">
          {!isCollapsed && (
            <div className="mb-4 px-2">
              <p className="text-sm font-medium text-white/90 truncate">{user?.name || 'Seller Store'}</p>
